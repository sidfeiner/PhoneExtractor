...
class FormatList(list):
	...
	def to_find_regex(self, strict = False, country_code = '', optional_country = False, canonized = True, stuck_zero = False):
		...
		"""
		return self._create_full_regex(absolute_anchors = False, strict = strict, country_code = country_code, optional_country=optional_country,
	...
	def to_exact_regex(self, strict = False, country_code = '', optional_country = False, canonized = True, stuck_zero = False):
		...
		"""
		return self._create_full_regex(absolute_anchors = True, strict = strict, country_code = country_code, optional_country=optional_country,
									   canonized = canonized, stuck_zero = stuck_zero)

	def _create_full_regex(self, absolute_anchors = True, strict = False, country_code = '', optional_country = False, canonized = True,
						   stuck_zero = False):
		"""
	...
	def _create_reversed_regex(self):
		...
		for index, format in enumerate(self):
			current_regex = "(?P<num{i}>(\\d{sep1}){{{min},{max}}}\\d)0*{sep2}(?P<pre{i}>{prefix})".format(
				prefix = format.prefix, sep1 = GENERAL_SEPARATOR, sep2 = OBLIGATED_SEPARATOR,
				min = format.min_length - 1, max = format.max_length - 1, i = len(basic_regexes))
	...
	def _create_regexes(self, canonized = True):
		...
		for format in self:
			if canonized:
				current_regex = "{prefix}\\d{{{min},{max}}}".format(prefix = format.prefix, min = format.min_length,
																	max = format.max_length)
			else:
				current_regex = "{prefix}{sep}(\\d{sep}){{{min},{max}}}\\d".format(prefix = format.prefix,
																				   sep = GENERAL_SEPARATOR,
																				   min = format.min_length - 1,
																				   max = format.max_length - 1)
			basic_regexes.append(current_regex)
...
class CountryPhone(object):
	def __init__(self, country = '', country_code = '', mobile_phone = Phone(), line_phone = Phone(),
		...
		if phones_dict:
			# init with dict
			self.mobile_phone, self.line_phone = self.__parse_phones_dict(phones_dict)
		else:
	...
	def get_phone_formats(self, strict = True, canonized = True):
	...
	def to_find_regex(self, strict = False, with_country = True, optional_country = False, canonized = True, stuck_zero = False):
		...
		Creates a final compiled regex that will find phone numbers
		"""
		return self._create_full_regex(absolute_anchors = False, strict = strict, with_country = with_country, optional_country = optional_country,
									   canonized = canonized, stuck_zero = stuck_zero)
	...
	def to_exact_regex(self, strict = False, with_country = True, optional_country = False, canonized = True, stuck_zero = False):
		...
		if strict or country_code are given as params, replace they are used instead of self.country_code and self._is_strict
		"""
		return self._create_full_regex(absolute_anchors = True, strict = strict, with_country = with_country, optional_country = optional_country,
									   canonized = canonized, stuck_zero = stuck_zero)
	...
	def __parse_phones_dict(self, phones_dict):
		"""
...
def create_israeli_phone():
	...
	return CountryPhone(country = 'israel', country_code = '972', phones_dict = israeli_formats)


def create_belgian_phone():
	"""
	...
	return CountryPhone(country = 'belgium', country_code = '32', phones_dict = belgian_formats)


def create_holland_phone():
	"""
	...
	return CountryPhone(country = 'france', country_code = '33', phones_dict = france_formats)

def create_all_phones():
	"""
	...
	phone_formats['israel'] = create_israeli_phone()
	phone_formats['belgium'] = create_belgian_phone()
	phone_formats['holland'] = create_holland_phone()
