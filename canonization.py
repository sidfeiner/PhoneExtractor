...
class CountryCanonizer(object):
	...
	def _init_regexes(self):
		...
		self._regexes_dict['only_numbers'] = (only_numbers_regex, '', 1)
		...
		self._regexes_dict['stuck_zeroes'] = (stuck_zeroes_regex, r'\1\2', 2)  # remove zeroes

		# Adds country code
		no_country_code_regex = re.compile(r'^({alt})$'.format(alt=alternatives))
		self._regexes_dict['no_country_code'] = (
			no_country_code_regex, "{country}\\1".format(country=self._country_phone.country_code), 3)

		# Reverse prefix and number
		reversing_regex = re.compile(self._create_reversed_regexes())
		self._regexes_dict['reversed'] = (reversing_regex, '\\g<pre{i}>\\g<num{i}>', 4)  # many capturing names
		...
		self._regexes_lst = [(item[0], item[1]) for item in temp_lst if
							 item[2] > 0]  # remove priority as its based on index now
	...
	def canonize(self, phone_number):
		...
		if success:
			...
			if success and simple_canon not in extracted:
				extracted.add(simple_canon)
	...
	def _try_extract(self, phone_number):
		...
		:rtype: tuple
		:return: extracted_number_list, code (0-simple canonized, 1-extracted, 2-none)
		"""
...
def create_israeli_canonizer():
	"""
	:return: israeli canonizer instance
	"""
	return CountryCanonizer(phone_formats.create_israeli_phone())

def create_belgian_canonizer():
	"""
	:return: belgian canonizer instance
	"""
	return CountryCanonizer(phone_formats.create_belgian_phone())

def create_holland_canonizer():
	"""
	:return: dutch canonizer instance
	"""
	return CountryCanonizer(phone_formats.create_holland_phone())

def create_all_canonizers():
	"""
	:return: dict of all canonizers
	"""

	canonizers = {}
	all_formats = phone_formats.create_all_phones()
